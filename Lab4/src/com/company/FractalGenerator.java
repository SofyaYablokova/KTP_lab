...
public abstract class FractalGenerator {
    ...
    public void recenterAndZoomRange(Rectangle2D.Double range, double centerX, double centerY, double scale){
        ...
        double newHeigth=range.height*scale;
        range.x=centerX-newWidth/2;
        range.y=centerY-newHeigth/2;
        range.width=newWidth;
        range.height=newHeigth;
    }
    /**
     * Given a coordinate <em>x</em> + <em>iy</em> in the complex plane,
     * computers and returns the number of iterations before the fractal
     * function escapes the bounding area for that point. A point that
     * doesn't escapes the before the iteration limit is reached is indicated
     * with a result of -1.
     */
